...
const LoginNew = ({ onLogin, onError }) => {
  ...
  const initializeGoogle = () => {
    ...
    if (!window.google || !window.google.accounts || !googleConfig?.clientId) {
      console.error("Missing dependencies for Google initialization");
      return;
    }
    ...
    try {
      ...
      // Render the button only when the user is on Google login tab
      if (loginMethod === "google") {
        setTimeout(() => {
          const buttonDiv = document.getElementById('google-signin-button');
          console.log("Attempting to render button in div:", buttonDiv);
          ...
          if (buttonDiv) {
            ...
            console.warn("Button div not found, user might not be on Google tab");
          }
        }, 300);
      }
    } catch (error) {
  ...
  // Initialize Google when config is loaded and on login method change
  useEffect(() => {
    if (googleConfig?.configured && googleConfig?.clientId) {
      console.log("Google config ready, checking Google services...");
  ...
  const showCustomGoogleButton = (buttonDiv) => {
    buttonDiv.innerHTML = `
      <div class="text-center py-8">
        ...
        <p class="text-xs text-red-600 mb-2">Google Sign-In not loading</p>
        <p class="text-xs text-gray-500">
          Check browser console for errors<br/>
          <strong>Use password login below</strong>
        </p>
      </div>
    `;
    
    document.getElementById('custom-google-signin').addEventListener('click', () => {
      setError("Google OAuth needs configuration. Add http://localhost:3000 as authorized JavaScript origin in Google Console. Use password login for now.");
